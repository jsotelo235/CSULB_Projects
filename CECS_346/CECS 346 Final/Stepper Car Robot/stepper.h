...
void Stepper_CW(uint32_t delay, uint8_t move);

// Move 1.8 degrees counterclockwise,
// delay is the time to wait after each step in bus cycles
void Stepper_CCW(uint32_t delay, uint8_t move);

// Move 1.8 degree right motor
void Stepper_RM(uint32_t delay, uint8_t move);

// Move 1.8 degree left motor
void Stepper_LM(uint32_t delay, uint8_t move);
