...
volatile uint32_t swPressed = 0;					// flag to keep track of button press
...
int main(void){
	...
	unsigned char inChar;								// keep track of the keyboard press
  ...
  while(1){
		...
		if(!swPressed && (inChar == 'r')){
			UART1_OutChar('b');							 // transmit data to MCU1
			ToggleLED();										 // function used to toggle LED	
		}
		else if(swPressed){
			GPIO_PORTF_DATA_R &= ~RED_LED;	// Red LED is off
			swPressed = 0;									// reset the swPressed flag
		}
		else if(inChar == '\0'){
			while(inChar != 'r' && flag == 0){
...
void ToggleLED(){
	...
	do{
		(Counts == 1) ? (GPIO_PORTF_DATA_R |= RED_LED) : (GPIO_PORTF_DATA_R &= ~RED_LED);
	}while(1 && !swPressed);
...
void SysTick_Handler(void){	
	Counts ^= 1;	// toogle flag to trigger an interrupt
}
