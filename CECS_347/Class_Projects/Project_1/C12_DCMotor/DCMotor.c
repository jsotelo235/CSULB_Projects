...
void Motor_Init(void){
  ...
  H = L = 40000;                			// 50%
  GPIO_PORTA_AMSEL_R	&= ~0x28;      	// disable analog functionality on PA5
  GPIO_PORTA_PCTL_R 	&= ~0x00F00000; // configure PA5 as GPIO
  GPIO_PORTA_DIR_R 		|= 0x28;    		// make PA5 out
  GPIO_PORTA_DR8R_R 	|= 0x28;    		// enable 8 mA drive on PA5
  GPIO_PORTA_AFSEL_R 	&= ~0x28;  			// disable alt funct on PA5
  GPIO_PORTA_DEN_R 		|= 0x28;     		// enable digital I/O on PA5
  GPIO_PORTA_DATA_R 	&= ~0x28;   		// make PA5 low
...
void SysTick_Handler(void){
  ...
  if(GPIO_PORTA_DATA_R&0x20 && GPIO_PORTA_DATA_R&0x08){   		// toggle PA5
		
    GPIO_PORTA_DATA_R &= ~0x20; 		// make PA5 low
		GPIO_PORTA_DATA_R &= ~0x08; 		// make PA3 low
...
void PortF_Init(void){  
  ...
  GPIO_PORTF_DIR_R 		&= ~0x11;    		// make PF4 and PF0 in (built-in button)
